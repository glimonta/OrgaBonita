...
leer1:  move $a0, $t0           #muevo file descriptor a a0
        ...
        blt $t4, 0x30, ErrorLectura     #si no es un numero => ERROR
        bgt $t4, 0x39, ErrorLectura     #si no es un numero => ERROR
...
pedirEspacio:   lb $t4, numCod          #cargamos el numCod en t4
                ...
                la $t5, 0($v0)          #cargamos en t5 la direccion de inicio
                                        #del espacio de memoria que nos asigna
                                        #el sistema
                sw $t5, codigos         #almacenamos en codigos esta direccion
                lw $t6, codigos         #cargamos en t6 la direccion de codigos
...
#############################################################################
        ...
        move $t0, $v0           #movemos a t0 el file descriptor
        ...
        move $a0, $t0           #movemos a a0 el file descriptor
...
finCargaHS:     move $a0, $t0           #cerramos el archivo
                li $v0, 16
                syscall
...
inic:   lb $t7, numInt          #cargamos el max de intentos en t7
        ...
        li $v0, 8               #lee los 10 bytes de la entrada y los
...
enLaUltima:     la $t1, punN            #cargo la direccion de punN en t1
                ...
                addi $t3, $t3, 1         #sumamos el punto correspondiente
                sb $t3, 1($t1)          #almacenamos en memoria
                b noAdivino             #vamos a noAdivino para imprimir el
                                        #nombre del jugador y su puntuacion
...
fin:    la $t1, punN            #cargamos en t1 la direccion de la puntuacion
        lb $s2, 0($t1)          #cargamos en s2 el byte de las decenas
        lb $s3, 1($t1)          #cargamos en s3 el byte de las unidades
        
        lb $s1, espacio         #cargamos un espacio en blando en s1
        la $t2, nuev            #cargamos en t2 la direccion de nuev (nuevo
                                #jugador)
        sb $s2, 0($t2)          #almacenamos el byte de las decenas en t2
        sb $s3, 1($t2)          #luego el byte de las unidades
        sb $s1, 2($t2)          #y finalmente el byte del espacio en blanco
        addi $t2, $t2, 3        #nos movemos 3 bytes en la direccion de t2
        la $t5, nombre          #cargamos en t5 la direccion del nombre
        
nuev0:  lb $t3, 0($t5)          #cargamos el byte actual
        sb $t3, 0($t2)          #lo almacenamos en t2 (nuev)
        
        addi $t5, $t5, 1        #nos movemos 1 byte en la direccion del nombre
        addi $t2, $t2, 1        #nos movemos 1 byte en la direccion de nuev
        
        bnez $t3, nuev0         #mientras no sea igual a cero seguimos en
                                #el ciclo copiando el nombre
        
        la $t5, nuev            #cargamos en t5 la direccion de nuev
        la $s5, punN            #cargamos en s5 la direccion de punN
        move $t4, $zero         #inicializamos el contador t4 en cero
        
cargarNuev:     li $t4, 3       #cargamos en t4 la cantidad de digitos que 
                                #tiene la puntuacion incluyendo el espacio
                la $t7, punN    #cargamos la direccion de la puntuacion en t7
                jal transf      #llamamos a la funcion transf
                
                sw $s0, punN    #almacenamos en punN el entero que nos devuelve
                                #la funcion
